# Readme, License etc.
set(GENERAL_FILES
        ${swift_SOURCE_DIR}/LICENSE
        ${swift_SOURCE_DIR}/LICENSE.html
        )
...
if(UNIX AND NOT APPLE)
    ...
    function(installQtLib path lib)
        # Follow symlink
    ...
    installQtLib(${QT_INSTALL_LIBS} libQt5Core.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5DBus.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5Gui.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5Multimedia.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5MultimediaWidgets.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5Network.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5OpenGL.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5Qml.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5Svg.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5Widgets.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5XcbQpa.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5Xml.so.5)
    installQtLib(${QT_INSTALL_LIBS} libQt5MultimediaGstTools.so.5)
