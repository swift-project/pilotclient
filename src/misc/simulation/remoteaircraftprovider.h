...
namespace swift::misc
{
    ...
    namespace simulation
    {
        ...
        public:
            ...
            remoteAircraftSituations(const aviation::CCallsign &callsign) const = 0;

            //! Average update time
            //! \threadsafe
            virtual MillisecondsMinMaxMean
            remoteAircraftSituationsTimestampDifferenceMinMaxMean(const aviation::CCallsign &callsign) const = 0;
            ...
            remoteAircraftSituationChanges(const aviation::CCallsign &callsign) const = 0;
            ...
            virtual bool updateAircraftModel(const aviation::CCallsign &callsign, const CAircraftModel &model,
                                             const CIdentifier &originator) = 0;
            ...
            virtual bool updateFastPositionEnabled(const aviation::CCallsign &callsign,
                                                   bool enableFastPositonUpdates) = 0;
            ...
            virtual int updateAircraftGroundElevation(const aviation::CCallsign &callsign,
                                                      ...
                                                      bool *updateAircraftGroundElevation) = 0;
            ...
            virtual aviation::CCallsignSet updateCGForModel(const QString &modelString,
                                                            const physical_quantities::CLength &cg) = 0;

            //! Update the CG and model string
            //! \threadsafe
            virtual bool updateCGAndModelString(const aviation::CCallsign &callsign,
                                                const physical_quantities::CLength &cg, const QString &modelString) = 0;
            ...
            //! \threadsafe
            virtual geo::CElevationPlane
            averageElevationOfNonMovingAircraft(const aviation::CAircraftSituation &reference,
                                                const physical_quantities::CLength &range, int minValues = 1,
                                                int sufficientValues = 2) const = 0;
...
namespace swift::misc::simulation
{
    ...
    public:
        ...
        void addReverseLookupMessage(const aviation::CCallsign &callsign, const QString &message,
    ...
    signals:
        ...
        //! Parts added
        void addedAircraftParts(const swift::misc::aviation::CCallsign &callsign,
    ...
    protected:
        ...
        //! \threadsafe
        CSimulatedAircraft addOrUpdateAircraftInRange(const aviation::CCallsign &callsign, const QString &aircraftIcao,
                                                      const QString &airlineIcao, const QString &livery,
                                                      const QString &modelString, CAircraftModel::ModelType modelType,
                                                      CStatusMessageList *log = nullptr);

        //! Update aircraft
        //! \threadsafe
        int updateAircraftInRange(const aviation::CCallsign &callsign, const CPropertyIndexVariantMap &vm,
                                  bool skipEqualValues = true);

        //! Update aircraft bearing, distance and situation
        //! \threadsafe
        //! \remark does NOT emit signals
        bool updateAircraftInRangeDistanceBearing(const aviation::CCallsign &callsign,
        ...
        //! \threadsafe
        virtual aviation::CAircraftSituation storeAircraftSituation(const aviation::CAircraftSituation &situation,
        ...
        //! \threadsafe
        void storeAircraftParts(const aviation::CCallsign &callsign, const aviation::CAircraftParts &parts,
                                bool removeOutdated);
        void storeAircraftParts(const aviation::CCallsign &callsign, const QJsonObject &jsonObject,
        ...
        bool guessOnGroundAndUpdateModelCG(aviation::CAircraftSituation &situation,
                                           const aviation::CAircraftSituationChange &change,
                                           const CAircraftModel &aircraftModel);

        //! Add an offset for testing
        aviation::CAircraftSituation
        addTestAltitudeOffsetToSituation(const aviation::CAircraftSituation &situation) const;
        ...
        static int setGroundElevationCheckedAndGuessGround(aviation::CAircraftSituationList &situations,
    ...
    private:
        ...
        aviation::CAircraftSituationListPerCallsign
            m_situationsByCallsign; //!< situations, for performance reasons per callsign, thread safe access required
        aviation::CAircraftSituationPerCallsign
            m_latestSituationByCallsign; //!< latest situations, for performance reasons per callsign, thread safe
                                         //!< access required
        aviation::CAircraftSituationPerCallsign
            m_latestOnGroundProviderElevation; //!< situations on ground with elevation from provider
        aviation::CAircraftPartsListPerCallsign
            m_partsByCallsign; //!< parts, for performance reasons per callsign, thread safe access required
        aviation::CAircraftSituationChangeListPerCallsign
            m_changesByCallsign; //!< changes, for performance reasons per callsign, thread safe access required (same
                                 //!< timestamps as corresponding situations)
        ...
        ReverseLookupLogging m_enableReverseLookupMsgs =
            RevLogSimplifiedInfo; //!< shall we log. information about the matching process
    ...
    public:
        ...
        aviation::CAircraftSituationChangeList
        remoteAircraftSituationChanges(const aviation::CCallsign &callsign) const;
