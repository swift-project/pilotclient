...
namespace swift::misc
{
    ...
    void doAfter(QFuture<T> future, QObject *context, F &&func)
    {
        private_ns::doAfter(future, context, std::forward<F>(func));
    }
    ...
    {
        private_ns::doAfter(future, context, [func = std::forward<F>(func)](auto &&) { func(); });
    }
    ...
    private:
        QSharedPointer<private_ns::CPromiseData<T>> m_data { new private_ns::CPromiseData<T> };
    };
