...
namespace swift::gui::models
{
    ...
    void CAtcStationListModel::setStationMode(CAtcStationListModel::AtcStationMode stationMode)
    {
        ...
        case StationsOnline:
        {
            ...
            m_columns.addColumn(
                CColumn("distance", CAtcStation::IndexRelativeDistance, new CAirspaceDistanceFormatter()));
            m_columns.addColumn(CColumn("r.", "in range", CAtcStation::IndexIsInRange,
                                        new CBoolIconFormatter("in range", "outside range")));
            m_columns.addColumn(CColumn("xc.", "AFV cross coupled", CAtcStation::IndexIsAfvCrossCoupled,
                                        new CBoolIconFormatter("AFV cross coupled", "not coupled")));
            m_columns.addColumn(CColumn("range", CAtcStation::IndexRange, new CAirspaceDistanceFormatter()));
            m_columns.addColumnIncognito(
                CColumn::standardString("controllername", { CAtcStation::IndexController, CUser::IndexRealName }));
            m_columns.addColumn(CColumn("until", "expected logoff time", CAtcStation::IndexLogoffTime,
