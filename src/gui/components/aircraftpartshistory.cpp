...
{
    CAircraftPartsHistory::CAircraftPartsHistory(QWidget *parent) : QFrame(parent), ui(new Ui::CAircraftPartsHistory)
    {
        ui->setupUi(this);
        ...
        connect(ui->comp_CallsignCompleter, &CCallsignCompleter::validChangedCallsignEntered, this,
                &CAircraftPartsHistory::callsignEntered);
        ...
        if (this->hasContexts())
        {
            connect(sGui->getIContextNetwork(), &IContextNetwork::changedLogOrDebugSettings, this,
                    &CAircraftPartsHistory::valuesChanged);
            connect(sGui->getIContextNetwork(), &IContextNetwork::connectionStatusChanged, this,
                    &CAircraftPartsHistory::connectionStatusChanged);
        }
    ...
    void CAircraftPartsHistory::initGui()
    {
        ...
        if (needCallsigns && !m_timerUpdateHistory.isActive()) { m_timerUpdateHistory.start(); }
        else if (!needCallsigns) { m_timerUpdateHistory.stop(); }
    ...
    bool CAircraftPartsHistory::partsHistoryEnabled() const { return this->hasContexts(); }
    ...
    void CAircraftPartsHistory::updatePartsHistory()
    {
        ...
        else
        {
            ...
            {
                html += u"<hr>" % aircraftPartsHistory.toHtml(CStatusMessageList::timestampHtmlOutput());
            }
        }
    ...
    void CAircraftPartsHistory::valuesChanged() { this->initGui(); }
    ...
    void CAircraftPartsHistory::toggleHistoryEnabled(bool enabled)
    {
        ...
        if (sender == ui->cb_PartsHistoryEnabled) { sGui->getIContextNetwork()->enableAircraftPartsHistory(enabled); }
    }
