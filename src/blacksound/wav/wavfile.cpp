...
//                m_fileFormat.setSampleFormat(qFromLittleEndian<quint16>(header.wave.bitsPerSample));

                if (header.wave.audioFormat == 1 || header.wave.audioFormat == 0)
                {
                    m_fileFormat.setSampleFormat(bps == 8 ? QAudioFormat::UInt8 : QAudioFormat::Int16); // TODO Always 16?
                }
                else
                {
