...
namespace swift::core
{
    ...
    CAircraftMatcher::~CAircraftMatcher() { this->saveDisabledForMatchingModels(); }
    ...
    CAirlineIcaoCode CAircraftMatcher::failoverValidAirlineIcaoDesignator(
        const CCallsign &callsign, const QString &primaryIcao, const QString &secondaryIcao, bool airlineFromCallsign,
        const QString &airlineName, const QString &airlineTelephony, bool useWebServices, CStatusMessageList *log)
    {
        CCallsign::addLogDetailsToList(
            log, callsign,
            QStringLiteral("Find airline designator from 1st: '%1' 2nd: '%2' callsign: '%3', use airline callsign: %4, "
                           "name: '%5' telephony: '%6' use web service: %7")
                .arg(primaryIcao, secondaryIcao, callsign.toQString(), boolToYesNo(airlineFromCallsign), airlineName,
                     airlineTelephony, boolToYesNo(useWebServices)),
            getLogCategories());
        ...
        do {
            if (CAircraftMatcher::isValidAirlineIcaoDesignator(primaryIcao, useWebServices))
            {
                CCallsign::addLogDetailsToList(log, callsign,
                                               QStringLiteral("Using primary airline ICAO '%1'").arg(primaryIcao),
                                               getLogCategories());
                code = stringToAirlineIcaoObject(callsign, primaryIcao, airlineName, airlineTelephony, useWebServices,
                                                 log);
                break;
            }
            if (CAircraftMatcher::isValidAirlineIcaoDesignator(secondaryIcao, useWebServices))
            {
                CCallsign::addLogDetailsToList(
                    log, callsign,
                    QStringLiteral("Using secondary airline ICAO '%1', primary '%2' not valid")
                        .arg(secondaryIcao, primaryIcao),
                    getLogCategories());
                code = stringToAirlineIcaoObject(callsign, secondaryIcao, airlineName, airlineTelephony, useWebServices,
                                                 log);
                break;
            }
            ...
            CCallsign::addLogDetailsToList(
                log, callsign,
                QStringLiteral("Two invalid airline ICAO codes (primary/secondary) '%1', '%2'")
                    .arg(primaryIcao, secondaryIcao),
                getLogCategories());
            if (airlineFromCallsign)
            {
                ...
                if (airlinePrefix.isEmpty() || flightNumber.isEmpty())
                {
                    CCallsign::addLogDetailsToList(
                        log, callsign,
                        QStringLiteral("Callsign '%1' cannot be split in airline '%1'/ flight number '%2'")
                            .arg(callsign.toQString(), flightNumber),
                        getLogCategories());
                    break;
                }
                ...
                {
                    CCallsign::addLogDetailsToList(log, callsign,
                                                   QStringLiteral("Using airline from callsign '%1', suffix: '%2'")
                                                       .arg(callsign.toQString(), airlinePrefix),
                                                   getLogCategories());
                    code = stringToAirlineIcaoObject(callsign, airlinePrefix, airlineName, airlineTelephony,
                                                     useWebServices, log);
                    break;
                }
            }
        ...
        {
            CCallsign::addLogDetailsToList(
                log, callsign, QStringLiteral("Resolved to airline designator: %1").arg(code.toQString(true)));
        }
        else
        {
            ...
            CCallsign::addLogDetailsToList(
                log, callsign,
                QStringLiteral("Cannot find airline designator from 1st: '%1' 2nd: '%2' callsign: '%3', use airline "
                               "callsign: %4, name: '%5' telephony: '%6' use web service: %7")
                    .arg(primaryIcao, secondaryIcao, callsign.toQString(), boolToYesNo(airlineFromCallsign),
                         airlineName, airlineTelephony, boolToYesNo(useWebServices)),
                getLogCategories());
        }
        return code;
    }
    ...
    CAirlineIcaoCode
    CAircraftMatcher::failoverValidAirlineIcaoDesignator(const CCallsign &callsign, const QString &primaryIcao,
                                                         const QString &secondaryIcao, bool airlineFromCallsign,
                                                         const QString &airlineName, const QString &airlineTelephony,
                                                         const CAircraftModelList &models, CStatusMessageList *log)
    {
        CCallsign::addLogDetailsToList(
            log, callsign,
            QStringLiteral("Find airline designator from 1st: '%1' 2nd: '%2' callsign: '%3', use airline callsign: %4, "
                           "airline name: '%5' telephony: '%6', models: %7")
                .arg(primaryIcao, secondaryIcao, callsign.toQString(), boolToYesNo(airlineFromCallsign), airlineName,
                     airlineTelephony, models.sizeString()),
            getLogCategories());
        ...
        do {
            ...
            if (!primaryIcao.isEmpty())
            {
                ...
                if (countPerAirline.size() == 1)
                {
                    ...
                    CCallsign::addLogDetailsToList(log, callsign,
                                                   QStringLiteral("Found only 1 airline ICAO '%1' in %2 models")
                                                       .arg(countPerAirline.firstKey().getDesignatorDbKey())
                                                       .arg(models.size()),
                                                   getLogCategories());
                    break;
                }
                ...
                if (!modelsWithAirline.isEmpty())
                {
                    ...
                    {
                        modelsWithAirline = CAircraftMatcher::ifPossibleReduceModelsByAirlineNameTelephonyDesignator(
                            callsign, airlineName, airlineTelephony, modelsWithAirline,
                            info.arg(modelsWithAirline.size()).arg(primaryIcao), reduced, log);
                    }
                    code = modelsWithAirline.getAirlineWithMaxCount();
                    CCallsign::addLogDetailsToList(log, callsign,
                                                   QStringLiteral("Using primary airline ICAO '%1' found '%2'")
                                                       .arg(primaryIcao, code.getDesignatorDbKey()),
                                                   getLogCategories());
                    break;
                }
            }
            ...
            if (!secondaryIcao.isEmpty())
            {
                ...
                if (!modelsWithAirline.isEmpty())
                {
                    if (modelsWithAirline.size() > 1)
                    {
                        modelsWithAirline = CAircraftMatcher::ifPossibleReduceModelsByAirlineNameTelephonyDesignator(
                            callsign, airlineName, airlineTelephony, modelsWithAirline,
                            info.arg(modelsWithAirline.size()).arg(secondaryIcao), reduced, log);
                    }
                    code = modelsWithAirline.getAirlineWithMaxCount();
                    CCallsign::addLogDetailsToList(log, callsign,
                                                   QStringLiteral("Using secondary airline ICAO '%1' found '%2'")
                                                       .arg(primaryIcao, code.getDesignatorDbKey()),
                                                   getLogCategories());
                    break;
                }
            ...
            if (airlineFromCallsign)
            {
                ...
                if (!modelsWithAirline.isEmpty())
                {
                    if (modelsWithAirline.size() > 1)
                    {
                        modelsWithAirline = CAircraftMatcher::ifPossibleReduceModelsByAirlineNameTelephonyDesignator(
                            callsign, airlineName, airlineTelephony, modelsWithAirline,
                            info.arg(modelsWithAirline.size()).arg(airlinePrefix), reduced, log);
                    }
                    code = modelsWithAirline.getAirlineWithMaxCount();
                    CCallsign::addLogDetailsToList(log, callsign,
                                                   QStringLiteral("Using callsign airline ICAO '%1' found '%2'")
                                                       .arg(airlinePrefix, code.getDesignatorDbKey()),
                                                   getLogCategories());
                    break;
                }
            }
        ...
        else
        {
            CCallsign::addLogDetailsToList(
                log, callsign,
                QStringLiteral("Cannot find airline designator from 1st: '%1' 2nd: '%2' callsign: '%3', use airline "
                               "callsign: %4, airline name: '%5' telephony: '%6', models: %7")
                    .arg(primaryIcao, secondaryIcao, callsign.toQString(), boolToYesNo(airlineFromCallsign),
                         airlineName, airlineTelephony, models.sizeString()),
                getLogCategories());
        }
        return code;
    }

    CAirlineIcaoCode CAircraftMatcher::failoverValidAirlineIcaoDesignatorModelsFirst(
        const CCallsign &callsign, const QString &primaryIcao, const QString &secondaryIcao, bool airlineFromCallsign,
        const QString &airlineName, const QString &airlineTelephony, const CAircraftModelList &models,
        CStatusMessageList *log)
    {
        if (!models.isEmpty())
        {
            CCallsign::addLogDetailsToList(
                log, callsign, QStringLiteral("Using %1 models to resolve airline designator").arg(models.size()));
            const CAirlineIcaoCode airline = CAircraftMatcher::failoverValidAirlineIcaoDesignator(
        ...
        CCallsign::addLogDetailsToList(log, callsign,
                                       QStringLiteral("Now using resolution of airline ICAO without specific models"));
        return CAircraftMatcher::failoverValidAirlineIcaoDesignator(
            callsign, primaryIcao, secondaryIcao, airlineFromCallsign, airlineName, airlineTelephony, true, log);
    }

    CAircraftModel CAircraftMatcher::getClosestMatch(const CSimulatedAircraft &remoteAircraft, MatchingLog whatToLog,
                                                     CStatusMessageList *log, bool useMatchingScript) const
    {
        ...
        static const QString summary("Matching summary\n"
                                     "-----------------------------------------\n"
                                     "- Combined: %1 -> %2\n"
                                     "- Aircraft: %3 -> %4\n"
                                     "- Airline:  %5 -> %6\n"
        ...
        CMatchingUtils::addLogDetailsToList(
            log, remoteAircraft,
            m2.arg(remoteAircraft.getCallsignAsString(),
                   removeSurroundingApostrophes(remoteAircraft.getModel().toQString())));
        CMatchingUtils::addLogDetailsToList(
            log, remoteAircraft,
            m3.arg(modelSet.size()).arg(modelSet.coverageSummaryForModel(remoteAircraft.getModel())));
        ...
        if (remoteAircraft.getModel().hasManuallySetString())
        {
            ...
            // no matching
            CMatchingUtils::addLogDetailsToList(
                log, remoteAircraft, u"Manually set model " % remoteAircraft.getModelString(), getLogCategories());
        ...
        else if (remoteAircraft.hasModelString())
        {
            ...
            if (setup.getMatchingMode().testFlag(CAircraftMatcherSetup::ByModelString))
            {
                ...
                {
                    CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                        u"Exact match by model string '" %
                                                            matchedModel.getModelStringAndDbKey() % "'",
                                                        getLogCategories(), CStatusMessage::SeverityError);
                    resolvedInPrephase = true;
                }
            }
            ...
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, "Skipping model string match",
                                                    getLogCategories());
            }
        }
        ...
        if (!resolvedInPrephase)
        {
            ...
            if (noString > 0 && log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, noModelStr.arg(noString));
            }
            ...
            if (setup.getMatchingMode().testFlag(CAircraftMatcherSetup::ExcludeNoDbData))
            {
                ...
                if (noDbKey > 0 && log)
                {
                    CMatchingUtils::addLogDetailsToList(log, remoteAircraft, excludedStr.arg(noDbKey));
                }
            }
            ...
            if (setup.getMatchingMode().testFlag(CAircraftMatcherSetup::ExcludeNoExcluded))
            {
                ...
                if (excluded > 0 && log)
                {
                    CMatchingUtils::addLogDetailsToList(log, remoteAircraft, excludedStr.arg(excluded));
                }
            }
            ...
            CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                msInfo.arg(setup.getMatchingAlgorithmAsString()).arg(modelSet.size()),
                                                getLogCategories());
            ...
            case CAircraftMatcherSetup::MatchingStepwiseReduce:
                candidates = CAircraftMatcher::getClosestMatchStepwiseReduceImplementation(
                    modelSet, setup, m_categoryMatcher, remoteAircraft, whatToLog, log);
                break;
            case CAircraftMatcherSetup::MatchingScoreBased:
                candidates = CAircraftMatcher::getClosestMatchScoreImplementation(modelSet, setup, remoteAircraft,
                                                                                  maxScore, whatToLog, log);
                break;
            case CAircraftMatcherSetup::MatchingStepwiseReducePlusScoreBased:
            default:
                candidates = CAircraftMatcher::getClosestMatchStepwiseReduceImplementation(
                    modelSet, setup, m_categoryMatcher, remoteAircraft, whatToLog, log);
                candidates = CAircraftMatcher::getClosestMatchScoreImplementation(candidates, setup, remoteAircraft,
                                                                                  maxScore, whatToLog, log);
                break;
            }
            ...
            {
                matchedModel = CAircraftMatcher::getCombinedTypeDefaultModel(modelSet, remoteAircraft,
                                                                             this->getDefaultModel(), whatToLog, log);
            }
            else
            {
                ...
                if (log)
                {
                    CMatchingUtils::addLogDetailsToList(
                        log, remoteAircraft,
                        QStringLiteral("Picking among %1 by strategy '%2'")
                            .arg(candidates.size())
                            .arg(CAircraftMatcherSetup::strategyToString(usedStrategy)));
                }
            }
        }
        ...
        {
            CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                QStringLiteral("Matching script: Matching stage script used"));
            const MatchingScriptReturnValues rv =
                CAircraftMatcher::matchingStageScript(remoteAircraft.getModel(), matchedModel, setup, modelSet, log);
            ...
            {
                CMatchingUtils::addLogDetailsToList(
                    log, remoteAircraft, QStringLiteral("Matching script: Modified values and re-run requested"));
                CMatchingUtils::addLogDetailsToList(
                    log, remoteAircraft,
                    QStringLiteral("Matching script: Now using model: '%1'").arg(matchedModel.toQString(true)));

                CSimulatedAircraft rerunAircraft(remoteAircraft);
                rerunAircraft.setModel(matchedModelMs);
                CStatusMessageList log2ndRun;
                matchedModelMs =
                    CAircraftMatcher::getClosestMatch(rerunAircraft, whatToLog, log ? &log2ndRun : nullptr, false);
                ...
                if (matchedModelMs.hasModelString()) { matchedModel = matchedModelMs; }
                else
                {
                    CMatchingUtils::addLogDetailsToList(
                        log, remoteAircraft,
                        QStringLiteral(
                            "Matching script: Ignoring model without model string after running the script"));
                }
            }
        }
        else
        {
            if (log)
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    QStringLiteral("Matching script: No matching stage script used"));
        }
        ...
        if (!matchedModel.hasModelString())
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    QStringLiteral("All matching yielded no result, VERY odd..."));
            }
            ...
            if (defaultModel.hasModelString())
            {
                ...
                if (log)
                {
                    CMatchingUtils::addLogDetailsToList(
                        log, remoteAircraft,
                        QStringLiteral("Using default model '%1'").arg(matchedModel.getModelString()));
                }
            }
            else
            {
                if (log)
                {
                    CMatchingUtils::addLogDetailsToList(
                        log, remoteAircraft,
                        QStringLiteral("Not even a default model. Giving up").arg(matchedModel.getModelString()));
                }
            }
        }
    ...
    CAircraftModel
    CAircraftMatcher::reverseLookupModel(const CCallsign &callsign, const CAircraftIcaoCode &networkAircraftIcao,
                                         const CAirlineIcaoCode &networkAirlineIcao, const QString &networkLiveryInfo,
                                         const QString &networkModelString, const CAircraftMatcherSetup &setup,
                                         const CAircraftModelList &modelSet, CAircraftModel::ModelType type,
                                         CStatusMessageList *log)
    {
        Q_UNUSED(setup)
    ...
    MatchingScriptReturnValues CAircraftMatcher::reverseLookupScript(const CAircraftModel &inModel,
                                                                     const CAircraftMatcherSetup &setup,
                                                                     const CAircraftModelList &modelSet,
                                                                     CStatusMessageList *log)
    {
        if (!setup.doRunMsReverseLookupScript()) { return { inModel }; }
        ...
        const MatchingScriptReturnValues rv =
            CAircraftMatcher::matchingScript(js, inModel, inModel, setup, modelSet, ReverseLookup, log);
        return rv;
    }

    MatchingScriptReturnValues CAircraftMatcher::matchingStageScript(const CAircraftModel &inModel,
                                                                     const CAircraftModel &matchedModel,
                                                                     const CAircraftMatcherSetup &setup,
                                                                     const CAircraftModelList &modelSet,
                                                                     CStatusMessageList *log)
    {
        if (!setup.doRunMsMatchingStageScript()) { return { inModel }; }
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return inModel; }
        const QString js = CFileUtils::readFileToString(setup.getMsMatchingStageFile());
        const MatchingScriptReturnValues rv =
            CAircraftMatcher::matchingScript(js, inModel, matchedModel, setup, modelSet, MatchingStage, log);
        return rv;
    }

    MatchingScriptReturnValues CAircraftMatcher::matchingScript(const QString &js, const CAircraftModel &inModel,
                                                                const CAircraftModel &matchedModel,
                                                                const CAircraftMatcherSetup &setup,
                                                                const CAircraftModelList &modelSet,
                                                                MatchingScript script, CStatusMessageList *log)
    {
        ...
        if (js.isEmpty() && log)
        {
            CCallsign::addLogDetailsToList(log, callsign, QStringLiteral("Matching script is empty"));
        }
        ...
        while (!js.isEmpty() && sApp && sApp->hasWebDataServices())
        {
            ...
            static const QString logFileR =
                CFileUtils::appendFilePaths(CSwiftDirectories::logDirectory(), "logMatchingScriptReverseLookup.log");
            static const QString logFileM =
                CFileUtils::appendFilePaths(CSwiftDirectories::logDirectory(), "logMatchingScriptMatchingStage.log");

            if (log)
            {
                CCallsign::addLogDetailsToList(
                    log, callsign, QStringLiteral("Matching script (%1): '%2'").arg(msToString(script), lf));
                CCallsign::addLogDetailsToList(
                    log, callsign,
                    QStringLiteral("Matching script input model (%1): '%2'").arg(inModel.toQString(true)));
                CCallsign::addLogDetailsToList(
                    log, callsign, QStringLiteral("Matching script models: %1").arg(modelSet.coverageSummary()));
            }
            ...
            // engine.globalObject().setProperty("SwiftWebServices", jsWebServicesMetaObject); // JS: new
            // SwiftWebServices
            ...
            if (ms.isError())
            {
                const QString msg = QStringLiteral("Matching script error: %1 '%2'")
                                        .arg(ms.property("lineNumber").toInt())
                                        .arg(ms.toString());
                ...
                if (ms.isQObject())
                {
                    ...
                    if (reverseModelProcessed->hasChangedModelId(inModel))
                    {
                        const CAircraftModel model =
                            sApp->getWebDataServices()->getModelForDbKey(reverseModelProcessed->getDbModelId());
                    ...
                    if (reverseModelProcessed->hasChangedModelString(inModel.getModelString()))
                    {
                        ...
                        const CAircraftModel modeSetModel =
                            CAircraftMatcher::reverseLookupModelStringInSet(modelString, callsign, modelSet, true, log);
                        if (modeSetModel.hasModelString())
                        {
                            CCallsign::addLogDetailsToList(
                                log, callsign,
                                QStringLiteral("Matching script using model from set: '%1'").arg(modelString));
                    ...
                    if (reverseModelProcessed->hasChangedAircraftIcao(matchedModel.getAircraftIcaoCode()))
                    {
                        ...
                        {
                            icao = sApp->getWebDataServices()->getAircraftIcaoCodeForDbKey(
                                reverseModelProcessed->getDbAircraftIcaoId());
                        }
                        rv.modified = true;
                    ...
                    if (reverseModelProcessed->hasChangedLiveryId(matchedModel.getLivery()))
                    {
                        const CLivery livery(
                            sApp->getWebDataServices()->getLiveryForDbKey(reverseModelProcessed->getDbLiveryId()));
                    ...
                    else if (reverseModelProcessed->hasChangedAirlineIcao(matchedModel.getAirlineIcaoCode()))
                    {
                        ...
                        {
                            icao = sApp->getWebDataServices()->getAirlineIcaoCodeForDbKey(
                                reverseModelProcessed->getDbAirlineIcaoId());
                        }
                        else
                        {
                            icao = sApp->getWebDataServices()->getAirlineIcaoCodeForUniqueDesignatorOrDefault(
                                reverseModelProcessed->getAirlineIcao(), true);
                        }
                        ...
                        if (log)
                        {
                            CCallsign::addLogDetailsToList(
                                log, callsign,
                                QStringLiteral("Matching script, changed airline ICAO: '%1' -> '%2'")
                                    .arg(matchedModel.getAirlineIcaoCode().toQString(true), icao.toQString(true)));
                        }
                        rv.modified = true;
                    }
                }
                else if (ms.isString()) { logMessage = ms.toString(); }
            }
        ...
        // log message
        if (log && !logMessage.isEmpty())
        {
            CCallsign::addLogDetailsToList(log, callsign, QStringLiteral("Matching script log: '%1'").arg(logMessage));
        }
    ...
    CAircraftModel CAircraftMatcher::reverseLookupModel(const CAircraftModel &modelToLookup,
                                                        const QString &networkLiveryInfo,
                                                        const CAircraftMatcherSetup &setup,
                                                        const CAircraftModelList &modelSet, CStatusMessageList *log)
    {
        ...
        // 3) This is not model matching here (!), it is a process of getting the most accurate data from that fuzzy
        // information I get via FSD
        //
        ...
        do {
            if (modelToLookup.hasModelString())
            {
                if (!setup.isReverseLookupModelString())
                {
                    if (log)
                    {
                        CCallsign::addLogDetailsToList(log, callsign, QStringLiteral("Model string looup disabled"));
                    }
                }
                else
                {
                    ...
                    const CAircraftModel modelFromDb = CAircraftMatcher::reverseLookupModelStringInDB(
                        modelString, callsign, setup.isReverseLookupModelString(), log);
                    ...
                    const CAircraftModel modelFromSet = CAircraftMatcher::reverseLookupModelStringInSet(
                        modelString, callsign, modelSet, useNonDbEntries, log);
            ...
            {
                CCallsign::addLogDetailsToList(log, callsign,
                                               QStringLiteral("Livery string with ids: '%1'").arg(ids.toQString()));
            }
            ...
            if (!setup.isReverseLookupSwiftLiveryIds())
            {
                if (log)
                {
                    CCallsign::addLogDetailsToList(
                        log, callsign,
                        QStringLiteral("Ignoring livery ids '%1', because of setup").arg(ids.toQString()));
                }
            }
            else if (ids.model >= 0 && !modelToLookup.hasModelString())
            {
                ...
                {
                    CCallsign::addLogDetailsToList(log, callsign,
                                                   QStringLiteral("Model lookup with id %1 from triple ids '%2'")
                                                       .arg(ids.model)
                                                       .arg(ids.toQString()));
                }
            ...
            if (!modelToLookup.getAircraftIcaoCode().hasValidDbKey() && ids.aircraft >= 0)
            {
                if (log)
                {
                    CCallsign::addLogDetailsToList(
                        log, callsign,
                        QStringLiteral("Aircraft ICAO lookup with id %1 from triple ids '%2'")
                            .arg(ids.aircraft)
                            .arg(ids.toQString()));
                }
                const CAircraftIcaoCode icaoFromDb =
                    CAircraftMatcher::reverseLookupAircraftIcaoId(ids.aircraft, callsign, log);
                if (icaoFromDb.hasValidDbKey()) { model.setAircraftIcaoCode(icaoFromDb); }
            }
            ...
            if (!modelToLookup.getLivery().hasValidDbKey() && ids.livery >= 0)
            {
                ...
                {
                    CCallsign::addLogDetailsToList(log, callsign,
                                                   QStringLiteral("Livery lookup with id %1 from triple ids '%2'")
                                                       .arg(ids.livery)
                                                       .arg(ids.toQString()));
                }
                const CLivery liveryFromDb = CAircraftMatcher::reverseLookupLiveryId(ids.livery, callsign, log);
                if (liveryFromDb.hasValidDbKey()) { model.setLivery(liveryFromDb); }
            }
            ...
            if (!model.getAircraftIcaoCode().hasValidDbKey())
            {
                ...
                if (!reverseIcaoCode.isLoadedFromDb())
                {
                    ...
                    if (reverseIcaoCode.isLoadedFromDb())
                    {
                        ...
                        {
                            CCallsign::addLogDetailsToList(log, callsign,
                                                           QStringLiteral("Set aircraft ICAO to '%1' from DB")
                                                               .arg(reverseIcaoCode.getCombinedIcaoStringWithKey()));
                        }
                    ...
                    else
                    {
                        ...
                        if (log)
                        {
                            CCallsign::addLogDetailsToList(
                                log, callsign,
                                QStringLiteral("Reverse lookup, ICAO '%1' not resolved from DB")
                                    .arg(reverseIcaoCode.getDesignator()));
                        }
                    }
                }
            }
            ...
            if (!model.getLivery().isLoadedFromDb())
            {
                ...
                if (CLivery::isValidCombinedCode(liveryCode))
                {
                    ...
                    if (reverseLivery.hasValidDbKey())
                    {
                        ...
                        {
                            CCallsign::addLogDetailsToList(log, callsign,
                                                           QStringLiteral("Reverse lookup of livery found '%1'")
                                                               .arg(reverseLivery.getCombinedCodePlusInfoAndId()),
                                                           getLogCategories());
                        }
                    }
                    else
                    {
                        ...
                        if (log)
                        {
                            CCallsign::addLogDetailsToList(
                                log, callsign,
                                QStringLiteral("Reverse lookup of livery '%1' yielded no result")
                                    .arg(reverseLivery.getCombinedCodePlusInfo()),
                                getLogCategories());
                        }
                    }
                } // livery lookup
                ...
                if (!model.hasValidDbKey() && !model.getLivery().hasValidDbKey())
                {
                    if (airlineIcaoCode.hasValidDesignator())
                    {
                        ...
                        {
                            const CLivery stdLivery(
                                sApp->getWebDataServices()->getStdLiveryForAirlineCode(airlineIcaoCode));
                            if (stdLivery.hasValidDbKey())
                            {
                                ...
                                {
                                    CCallsign::addLogDetailsToList(log, callsign,
                                                                   QStringLiteral("Set standardlivery `%1`")
                                                                       .arg(stdLivery.getCombinedCodePlusInfo()));
                                }
                            }
                        }
                        ...
                        if (!model.getLivery().hasValidDbKey())
                        {
                            ...
                            const CLivery liveryDummy(CLivery::getStandardCode(airlineIcaoCode), airlineIcaoCode,
                                                      "Generated");
                            model.setLivery(liveryDummy);
                            if (log)
                            {
                                CCallsign::addLogDetailsToList(log, callsign,
                                                               QStringLiteral("Generated livery, set livery `%1`")
                                                                   .arg(liveryDummy.getCombinedCodePlusInfo()));
                            }
                        }
                    }
                } // pseudo livery
        ...
        {
            CCallsign::addLogDetailsToList(log, callsign,
                                           QStringLiteral("Using model: ICAO '%1', livery '%2', model '%3', type '%4'")
                                               .arg(model.getAircraftIcaoCode().getCombinedIcaoStringWithKey(),
                                                    model.getLivery().getCombinedCodePlusInfo(), model.getModelString(),
                                                    model.getModelTypeAsString()));
        }
        return model;
    }

    CAircraftModel CAircraftMatcher::reverseLookupModelMs(const CAircraftModel &modelToLookup,
                                                          const QString &networkLiveryInfo,
                                                          const CAircraftMatcherSetup &setup,
                                                          const CAircraftModelList &modelSet, CStatusMessageList *log)
    {
        ...
        if (rv.runScriptModifiedAndRerun())
        {
            CCallsign::addLogDetailsToList(log, cs,
                                           QStringLiteral("Matching script: Modified value and requested rerun"));
    ...
    CAircraftModel CAircraftMatcher::reverseLookupModelStringInDB(const QString &modelString, const CCallsign &callsign,
                                                                  bool doLookupString, CStatusMessageList *log)
    {
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return {}; }
        if (!doLookupString)
        {
            if (log)
            {
                CCallsign::addLogDetailsToList(
                    log, callsign,
                    QStringLiteral("Ignore model string in reverse lookup (disabled), ignoring '%1'").arg(modelString));
            }
            return {};
        }
        ...
        if (log)
        {
            if (isDBModel)
            {
                CCallsign::addLogDetailsToList(
                    log, callsign,
                    QStringLiteral("Found model in DB for model string '%1' dist: '%2' descr.: '%3'")
                        .arg(model.getModelStringAndDbKey(), model.getDistributor().getDbKey(),
                             model.getDescription()));
            }
            else
            {
                CCallsign::addLogDetailsToList(
                    log, callsign, QStringLiteral("Did not find model in DB for model string '%1'").arg(modelString));
            }
        }

        if (!isDBModel) { return {}; } // not found
    ...
    CAircraftModel CAircraftMatcher::reverseLookupModelStringInSet(const QString &modelString,
                                                                   const CCallsign &callsign,
                                                                   const CAircraftModelList &modelSet,
                                                                   bool useNonDbEntries, CStatusMessageList *log)
    {
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return {}; }
        if (modelString.isEmpty())
        {
            ...
            {
                CCallsign::addLogDetailsToList(
                    log, callsign, QStringLiteral("Empty model string for lookup in %1 models").arg(modelSet.size()));
            }
            return {};
        }
        if (modelSet.isEmpty())
        {
            ...
            {
                CCallsign::addLogDetailsToList(log, callsign,
                                               QStringLiteral("Empty models, ignoring '%1'").arg(modelString));
            }
            return {};
        }
        ...
        if (!model.hasModelString())
        {
            if (log)
            {
                CCallsign::addLogDetailsToList(
                    log, callsign,
                    QStringLiteral("Model '%1' not found in %2 models").arg(modelString).arg(modelSet.size()));
            }
            return {};
        }
        ...
        if (log)
        {
            ...
            {
                CCallsign::addLogDetailsToList(log, callsign,
                                               QStringLiteral("Found DB model in %1 models for model string '%2'")
                                                   .arg(modelSet.size())
                                                   .arg(model.getModelStringAndDbKey()));
            }
            else
            {
                CCallsign::addLogDetailsToList(log, callsign,
                                               QStringLiteral("Found NON DB model in %1 models for model string '%2'")
                                                   .arg(modelSet.size())
                                                   .arg(model.getModelString()));
            }
        }

        if (!isDBModel && !useNonDbEntries) { return {}; } // ignore DB entries
    ...
    CAircraftModel CAircraftMatcher::reverseLookupModelId(int id, const CCallsign &callsign, CStatusMessageList *log)
    {
        ...
        if (log)
        {
            ...
            {
                CCallsign::addLogDetailsToList(log, callsign,
                                               QStringLiteral("Did not find model in DB for id '%1'").arg(id));
            }
        }
        model.setCallsign(callsign);
        model.setModelType(CAircraftModel::TypeReverseLookup);
    ...
    CAircraftIcaoCode CAircraftMatcher::reverseLookupAircraftIcao(const CAircraftIcaoCode &icaoCandidate,
                                                                  const CCallsign &logCallsign, CStatusMessageList *log)
    {
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return {}; }
        ...
        if (foundIcaos.isEmpty())
        {
            ...
            if (CAircraftIcaoCode::isValidDesignator(designator))
            {
                CCallsign::addLogDetailsToList(
                    log, logCallsign,
                    QStringLiteral("Reverse lookup of aircraft ICAO '%1' did not find anything, using smart search")
                        .arg(designator),
            ...
            else
            {
                CCallsign::addLogDetailsToList(
                    log, logCallsign,
                    QStringLiteral("Reverse lookup of invalid ICAO code '%1' did not find anything so far")
                        .arg(designator),
                    CAircraftMatcher::getLogCategories());
                ...
                for (const QString &p : parts)
                {
                    CCallsign::addLogDetailsToList(
                        log, logCallsign,
                        QStringLiteral("Trying parts, now reverse lookup of aircraft ICAO '%1' using smart search")
                            .arg(p),
                        CAircraftMatcher::getLogCategories());
            ...
            {
                CCallsign::addLogDetailsToList(log, logCallsign,
                                               QStringLiteral("No DB data for ICAO '%1', valid ICAO?").arg(designator),
                                               CAircraftMatcher::getLogCategories());
                return { icaoCandidate };
            }
        }
        ...
        {
            CCallsign::addLogDetailsToList(
                log, logCallsign, QStringLiteral("Reverse lookup of aircraft ICAO '%1', nothing found").arg(designator),
                CAircraftMatcher::getLogCategories());
            return { icaoCandidate };
        }
        else if (foundIcaos.size() == 1)
        {
            ...
            CCallsign::addLogDetailsToList(
                log, logCallsign,
                QStringLiteral("Reverse lookup of aircraft ICAO '%1', found one manufacturer '%2' in DB")
                    .arg(designator, icao.getDesignatorManufacturer()),
                CAircraftMatcher::getLogCategories());
            return icao;
        }
        else
        {
            ...
            CCallsign::addLogDetailsToList(
                log, logCallsign,
                QStringLiteral("Reverse lookup of aircraft ICAO '%1', found %2 values (ambiguous): %3")
                    .arg(designator)
                    .arg(foundIcaos.size())
                    .arg(foundIcaos.dbKeysAsString(", ")),
                CAircraftMatcher::getLogCategories());
            if (maxManufacturer.second < foundIcaos.size())
            {
                foundIcaos = foundIcaos.findByManufacturer(maxManufacturer.first);
                CCallsign::addLogDetailsToList(log, logCallsign,
                                               QStringLiteral("Reducing by manufacturer '%1', now %2 values")
                                                   .arg(maxManufacturer.first)
                                                   .arg(foundIcaos.size()),
                                               CAircraftMatcher::getLogCategories());
            }
            ...
            CCallsign::addLogDetailsToList(
                log, logCallsign,
                QStringLiteral("Reverse lookup of aircraft ICAO '%1', using ICAO '%2' with rank %3")
                    .arg(designator, icao.toQString(), icao.getRankString()),
                CAircraftMatcher::getLogCategories());
            return icao;
        }
    }

    CAircraftIcaoCode CAircraftMatcher::reverseLookupAircraftIcaoId(int id, const CCallsign &logCallsign,
                                                                    CStatusMessageList *log)
    {
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return {}; }
        ...
        if (log)
        {
            ...
            {
                CCallsign::addLogDetailsToList(log, logCallsign,
                                               QStringLiteral("Found aircraft ICAO in DB for id '%1'").arg(id));
            }
            else
            {
                CCallsign::addLogDetailsToList(log, logCallsign,
                                               QStringLiteral("Did not find aircraft ICAO in DB for id '%1'").arg(id));
            }
        }
        return icao;
    }

    CAirlineIcaoCode CAircraftMatcher::reverseLookupAirlineIcao(const CAirlineIcaoCode &icaoPattern,
                                                                const CCallsign &callsign, CStatusMessageList *log)
    {
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return {}; }
        ...
        if (log)
        {
            if (icao.hasValidDbKey())
            {
                CCallsign::addLogDetailsToList(
                    log, callsign,
                    QStringLiteral("Reverse lookup of airline ICAO '%1' and callsign '%2' found '%3' '%4' in DB")
                        .arg(icaoPattern.getDesignator(), callsign.asString(), icao.getVDesignatorDbKey(),
                             icao.getName()),
                    CAircraftMatcher::getLogCategories());
            }
            else
            {
                CCallsign::addLogDetailsToList(
                    log, callsign,
                    QStringLiteral("Reverse lookup of airline ICAO '%1' and callsign '%2', nothing found in DB")
                        .arg(icaoPattern.getDesignator(), callsign.asString()),
                    CAircraftMatcher::getLogCategories());
            }
        }
        return icao;
    ...
    CLivery CAircraftMatcher::reverseLookupStandardLivery(const CAirlineIcaoCode &airline, const CCallsign &callsign,
                                                          CStatusMessageList *log)
    {
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return {}; }
        if (!airline.hasValidDesignator())
        {
            ...
            {
                CCallsign::addLogDetailsToList(
                    log, callsign, QStringLiteral("Reverse lookup of standard livery skipped, no airline designator"),
                    CAircraftMatcher::getLogCategories(), CStatusMessage::SeverityWarning);
            }
            return {};
        }
        ...
        if (log)
        {
            ...
            {
                CCallsign::addLogDetailsToList(log, callsign,
                                               QStringLiteral("Reverse lookup of standard livery for '%1' found '%2'")
                                                   .arg(airline.getDesignator(), livery.getCombinedCode()),
                                               CAircraftMatcher::getLogCategories());
            }
            else
            {
                CCallsign::addLogDetailsToList(
                    log, callsign,
                    QStringLiteral("Not standard livery for airline '%1' in DB").arg(airline.getDesignator()),
                    CAircraftMatcher::getLogCategories());
            }
        }
        return livery;
    ...
    CLivery CAircraftMatcher::reverseLookupLiveryId(int id, const CCallsign &logCallsign, CStatusMessageList *log)
    {
        ...
        if (log)
        {
            ...
            {
                CCallsign::addLogDetailsToList(log, logCallsign,
                                               QStringLiteral("Found livery in DB for id '%1'").arg(id));
            }
            else
            {
                CCallsign::addLogDetailsToList(log, logCallsign,
                                               QStringLiteral("Did not find livery in DB for id '%1'").arg(id));
            }
        }
        return livery;
    }

    int CAircraftMatcher::reverseLookupByIds(const DBTripleIds &ids, CAircraftIcaoCode &aircraftIcao, CLivery &livery,
                                             const CCallsign &logCallsign, CStatusMessageList *log)
    {
        int found = 0;
        if (ids.livery >= 0)
    ...
    QString CAircraftMatcher::reverseLookupAirlineName(const QString &candidate, const CCallsign &callsign,
                                                       CStatusMessageList *log)
    {
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return {}; }
        ...
        {
            CCallsign::addLogDetailsToList(log, callsign,
                                           QStringLiteral("Airline name '%1' found in DB").arg(candidate));
            return candidate;
        }

        CCallsign::addLogDetailsToList(log, callsign,
                                       QStringLiteral("Airline name '%1' not found in DB").arg(candidate));
        return {};
    }

    QString CAircraftMatcher::reverseLookupTelephonyDesignator(const QString &candidate, const CCallsign &callsign,
                                                               CStatusMessageList *log)
    {
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return {}; }
        if (candidate.isEmpty()) { return {}; }
    ...
    bool CAircraftMatcher::isKnownAircraftDesignator(const QString &candidate, const CCallsign &callsign,
                                                     CStatusMessageList *log)
    {
        ...
        {
            CCallsign::addLogDetailsToList(log, callsign,
                                           QStringLiteral("No valid ICAO designator '%1'").arg(candidate));
            return false;
        }
    ...
    bool CAircraftMatcher::isKnownModelString(const QString &candidate, const CCallsign &callsign,
                                              CStatusMessageList *log)
    {
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return false; }
    ...
    CAircraftIcaoCode CAircraftMatcher::searchAmongAirlineAircraft(const QString &candidateString,
                                                                   ...
                                                                   const CCallsign &callsign, CStatusMessageList *log)
    {
        ...
        {
            CCallsign::addLogDetailsToList(
                log, callsign, QStringLiteral("No valid airline from DB '%1'").arg(airline.getDesignator()));
            return {};
        }
        ...
        {
            CCallsign::addLogDetailsToList(
                log, callsign, QStringLiteral("No aircraft known for airline '%1'").arg(airline.getDesignator()));
            return {};
        }
        ...
        CCallsign::addLogDetailsToList(
            log, callsign,
            QStringLiteral("Aircraft '%1' known for airline '%2'").arg(allIcaosStr, airline.getDesignator()));

        const CAircraftIcaoCode code = aircraft.findBestFuzzyMatchOrDefault(candidateString);
        if (code.hasValidDesignator())
        {
            CCallsign::addLogDetailsToList(log, callsign,
                                           QStringLiteral("Aircraft '%1' is best fuzzy search of '%2' for airline '%3'")
                                               .arg(code.toQString(), candidateString, airline.getDesignator()));
            return code;
        }
    ...
    {
        if (callsign.isEmpty() || !sApp || !sApp->getWebDataServices()) { return {}; }
        ...
        if (icao.hasValidDesignator())
        {
            CCallsign::addLogDetailsToList(
                log, callsign,
                QStringLiteral("Turned callsign %1 into airline %2").arg(callsign.asString(), icao.getDesignator()),
                getLogCategories());
        }
        else
        {
            CCallsign::addLogDetailsToList(
                log, callsign, QStringLiteral("Cannot turn callsign %1 into airline").arg(callsign.asString()),
                getLogCategories());
        }
        return icao;
    }
    ...
    int CAircraftMatcher::setModelSet(const CAircraftModelList &models, const CSimulatorInfo &simulator, bool forced)
    {
        ...
        {
            warnings =
                QStringLiteral("Removed models for matcher, without string #: %1, excluded #: %2.").arg(r1).arg(r2);
            if (r1 > 0)
            {
                warnings += QStringLiteral(" Without string: '%1'.")
                                .arg(models.findEmptyModelStrings().getModelStringList().join(", "));
            }
            if (r2 > 0)
            {
                warnings += QStringLiteral(" Excluded: '%1'.")
                                .arg(models.findByModelMode(CAircraftModel::Exclude).getModelStringList().join(", "));
            }
        }
    ...
    void CAircraftMatcher::evaluateStatisticsEntry(const QString &sessionId, const CCallsign &callsign,
                                                   ...
                                                   const QString &livery)
    {
        ...
        if (airlineIcao.isEmpty())
        {
            ...
            if (al.isLoadedFromDb()) { airlineIcaoChecked = al; }
        }
    ...
    void CAircraftMatcher::addingRemoteModelFailed(const CSimulatedAircraft &remoteAircraft)
    {
        ...
        if (remoteAircraft.hasCallsign() && remoteAircraft.hasModelString())
        {
            ...
            CLogMessage(this).warning(u"Disabled CS: '%1' model '%2' for matching")
                << remoteAircraft.getCallsignAsString() << modelString;
        }
        else { CLogMessage(this).warning(u"Disabled '%1' for matching") << remoteAircraft.toQString(true); }
    }
    ...
    bool CAircraftMatcher::saveDisabledForMatchingModels()
    {
        ...
        return CFileUtils::writeStringToFile(
            json, CFileUtils::appendFilePathsAndFixUnc(CSwiftDirectories::logDirectory(),
                                                       QStringLiteral("removed models %1.json").arg(ts)));
    }

    CAircraftModelList CAircraftMatcher::getClosestMatchStepwiseReduceImplementation(
        const CAircraftModelList &modelSet, const CAircraftMatcherSetup &setup, const CCategoryMatcher &categoryMatcher,
        const CSimulatedAircraft &remoteAircraft, MatchingLog whatToLog, CStatusMessageList *log)
    {
        ...
        do {
            ...
            {
                matchedModels =
                    ifPossibleReduceByLiveryAndAircraftIcaoCode(remoteAircraft, matchedModels, reduced, log);
                if (reduced) { break; } // almost perfect, we stop here (we have ICAO + livery match)
            }
            else if (reduceLog)
            {
                CMatchingUtils::addLogDetailsToList(reduceLog, remoteAircraft,
                                                    QStringLiteral("Skipping livery reduction"), getLogCategories());
            }
            ...
            {
                CMatchingUtils::addLogDetailsToList(reduceLog, remoteAircraft,
                                                    QStringLiteral("Skipping ICAO reduction"), getLogCategories());

                // family only because aircraft ICAO is not used
                if (mode.testFlag(CAircraftMatcherSetup::ByFamily))
                {
                    QString usedFamily;
                    matchedModels = ifPossibleReduceByFamily(remoteAircraft, UsePseudoFamily, matchedModels, reduced,
                                                             usedFamily, log);
                    if (reduced) { break; }
                }
                else if (reduceLog)
                {
                    CMatchingUtils::addLogDetailsToList(reduceLog, remoteAircraft,
                                                        QStringLiteral("Skipping family match"), getLogCategories());
                }
            }

            if (setup.useCategoryMatching())
            {
                matchedModels = categoryMatcher.reduceByCategories(matchedModels, modelSet, setup, remoteAircraft,
                                                                   reduced, whatToLog, log);
                // ?? break here ??
            }
            else if (reduceLog)
            {
                CMatchingUtils::addLogDetailsToList(reduceLog, remoteAircraft,
                                                    QStringLiteral("category matching disabled"), getLogCategories());
            }

            // if not yet reduced, reduce to VTOL
            if (!reduced && remoteAircraft.isVtol() && matchedModels.containsVtol() &&
                mode.testFlag(CAircraftMatcherSetup::ByVtol))
            {
                matchedModels = matchedModels.findByVtolFlag(true);
                CMatchingUtils::addLogDetailsToList(
                    log, remoteAircraft, QStringLiteral("Aircraft is VTOL, reduced to VTOL"), getLogCategories());
            }
            ...
            {
                matchedModels =
                    ifPossibleReduceByCombinedType(remoteAircraft, matchedModels, setup, reduced, reduceLog);
                if (reduced) { break; }
            }
            else if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, QStringLiteral("Skipping combined code match"),
                                                    getLogCategories());
            }
        }
        while (false);
        ...
        {
            matchedModels = ifPossibleReduceByManufacturer(remoteAircraft, matchedModels,
                                                           QStringLiteral("2nd trial to reduce by manufacturer. "),
                                                           reduced, reduceLog);
        }
    ...
    CAircraftModelList CAircraftMatcher::getClosestMatchScoreImplementation(const CAircraftModelList &modelSet,
                                                                            const CAircraftMatcherSetup &setup,
                                                                            const CSimulatedAircraft &remoteAircraft,
                                                                            int &maxScore, MatchingLog whatToLog,
                                                                            CStatusMessageList *log)
    {
        ...
        CMatchingUtils::addLogDetailsToList(scoreLog, remoteAircraft,
                                            QStringLiteral("Scores: %1").arg(scoresToString(map)), getLogCategories());
        CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                            QStringLiteral("Scoring with score %1 out of %2 models yielded %3 models")
                                                .arg(maxScore)
                                                .arg(map.size())
                                                .arg(maxScoreAircraft.size()),
                                            getLogCategories());
        return maxScoreAircraft;
    }

    CAircraftModel CAircraftMatcher::getCombinedTypeDefaultModel(const CAircraftModelList &modelSet,
                                                                 const CSimulatedAircraft &remoteAircraft,
                                                                 const CAircraftModel &defaultModel,
                                                                 MatchingLog whatToLog, CStatusMessageList *log)
    {
        ...
        {
            CMatchingUtils::addLogDetailsToList(combinedLog, remoteAircraft,
                                                QStringLiteral("No combined type, using default"), getLogCategories(),
                                                CStatusMessage::SeverityInfo);
            return defaultModel;
        }
        if (modelSet.isEmpty())
        {
            CMatchingUtils::addLogDetailsToList(combinedLog, remoteAircraft, QStringLiteral("No models, using default"),
                                                getLogCategories(), CStatusMessage::SeverityError);
            return defaultModel;
        }

        CMatchingUtils::addLogDetailsToList(combinedLog, remoteAircraft,
                                            u"Searching by combined type with color livery '" % combinedType % "'",
                                            getLogCategories());
        CAircraftModelList matchedModels = modelSet.findByCombinedTypeWithColorLivery(combinedType);
        if (!matchedModels.isEmpty())
        {
            CMatchingUtils::addLogDetailsToList(combinedLog, remoteAircraft,
                                                u"Found " % QString::number(matchedModels.size()) %
                                                    u" by combined type w/color livery '" % combinedType % "'",
                                                getLogCategories());
        }
        else
        {
            CMatchingUtils::addLogDetailsToList(
                combinedLog, remoteAircraft, u"Searching by combined type '" % combinedType % "'", getLogCategories());
            matchedModels = matchedModels.findByCombinedType(combinedType);
            if (!matchedModels.isEmpty())
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    u"Found " % QString::number(matchedModels.size()) %
                                                        u" by combined '" % combinedType % "'",
                                                    getLogCategories());
            }
        }
    ...
    CAircraftModel CAircraftMatcher::matchByExactModelString(const CSimulatedAircraft &remoteAircraft,
                                                             const CAircraftModelList &models, MatchingLog whatToLog,
                                                             CStatusMessageList *log)
    {
        ...
        if (remoteAircraft.getModelString().isEmpty())
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    QStringLiteral("No model string, no exact match possible"));
            }
            return {};
        }
        ...
        if (msLog)
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(
                    msLog, remoteAircraft, QStringLiteral("Found exact match for '%1'").arg(model.getModelString()));
            }
            else
            {
                CMatchingUtils::addLogDetailsToList(
                    msLog, remoteAircraft,
                    QStringLiteral("No exact match for '%1'").arg(remoteAircraft.getModelString()));
            }
        }
    ...
    CAircraftModelList
    CAircraftMatcher::ifPossibleReduceByLiveryAndAircraftIcaoCode(const CSimulatedAircraft &remoteAircraft,
                                                                  const CAircraftModelList &inList, bool &reduced,
                                                                  CStatusMessageList *log)
    {
        reduced = false;
        if (!remoteAircraft.getLivery().hasCombinedCode())
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(
                    log, remoteAircraft, QStringLiteral("No livery code, no reduction possible"), getLogCategories());
            }
            return inList;
        }

        const CAircraftModelList byLivery(inList.findByAircraftDesignatorAndLiveryCombinedCode(
            remoteAircraft.getLivery().getCombinedCode(), remoteAircraft.getAircraftIcaoCodeDesignator()));
        ...
        if (byLivery.isEmpty())
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(
                    log, remoteAircraft, u"Not found by livery code " % remoteAircraft.getLivery().getCombinedCode(),
                    getLogCategories());
            }
            return inList;
        }
    ...
    CAircraftModelList CAircraftMatcher::ifPossibleReduceByIcaoData(const CSimulatedAircraft &remoteAircraft,
                                                                    const CAircraftModelList &inList,
                                                                    const CAircraftMatcherSetup &setup, bool &reduced,
                                                                    CStatusMessageList *log)
    {
        const CAircraftMatcherSetup::MatchingMode mode = setup.getMatchingMode();
        if (inList.isEmpty())
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, QStringLiteral("Empty list, skipping step"),
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        if (mode.testFlag(CAircraftMatcherSetup::ByIcaoOrderAirlineFirst))
        {
            bool r1 = false;
            bool r2 = false;
            CAircraftModelList models = ifPossibleReduceByAirline(remoteAircraft, inList, setup,
                                                                  QStringLiteral("Reduce by airline first."), r1, log);
            models = ifPossibleReduceByAircraftOrFamily(remoteAircraft, UsePseudoFamily, models, setup,
                                                        QStringLiteral("Reduce by aircraft ICAO second."), r2, log);
            reduced = r1 || r2;
        ...
        else if (mode.testFlag(CAircraftMatcherSetup::ByIcaoData))
        {
            ...
            CAircraftModelList models =
                ifPossibleReduceByAircraftOrFamily(remoteAircraft, UsePseudoFamily, inList, setup,
                                                   QStringLiteral("Reduce by aircraft ICAO first."), r1, log);
            models = ifPossibleReduceByAirline(remoteAircraft, models, setup,
                                               QStringLiteral("Reduce aircraft ICAO by airline second."), r2, log);
            ...
            if (!remoteAircraft.hasAirlineDesignator())
            {
                ...
                {
                    CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                        "No airline, no secondary search for airline/aircraft",
                                                        getLogCategories());
                }
            }
            else if (!r2 && mode.testFlag(CAircraftMatcherSetup::ByFamily))
            {
                ...
                {
                    CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                        u"No exact ICAO match of '" %
                                                            remoteAircraft.getAirlineAndAircraftIcaoCodeDesignators() %
                                                            u"', will try family combination",
                                                        getLogCategories());
                }

                bool r3 = false;
                QString usedFamily;
                CAircraftModelList models2nd =
                    ifPossibleReduceByFamily(remoteAircraft, UsePseudoFamily, inList, r3, usedFamily, log);
                models2nd = ifPossibleReduceByAirline(remoteAircraft, models2nd, setup,
                                                      "Reduce family by airline second.", r3, log);
                if (r3)
                {
                    ...
                    {
                        CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                            u"Found " % QString::number(models2nd.sizeInt()) %
            ...
            if (reduced)
            {
                ...
                {
                    CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                        u"Reduced by aircraft ICAO: " % boolToYesNo(r1) %
                                                            u" airline: " % boolToYesNo(r2),
                                                        getLogCategories());
                }
                return models;
            }
        ...
        if (log)
        {
            CMatchingUtils::addLogDetailsToList(log, remoteAircraft, QStringLiteral("No reduction by ICAO data"),
                                                getLogCategories());
        }
        return inList;
    }

    CAircraftModelList CAircraftMatcher::ifPossibleReduceByFamily(const CSimulatedAircraft &remoteAircraft,
                                                                  bool allowPseudoFamily,
                                                                  const CAircraftModelList &inList, bool &reduced,
                                                                  QString &usedFamily, CStatusMessageList *log)
    {
        reduced = false;
        usedFamily = remoteAircraft.getAircraftIcaoCode().getFamily();
        if (!usedFamily.isEmpty())
        {
            CAircraftModelList matchedModels =
                ifPossibleReduceByFamily(remoteAircraft, usedFamily, allowPseudoFamily, inList,
                                         QStringLiteral("real family from ICAO"), reduced, log);
        ...
        return ifPossibleReduceByFamily(remoteAircraft, usedFamily, allowPseudoFamily, inList,
                                        QStringLiteral("ICAO treated as family"), reduced, log);
    }

    CAircraftModelList CAircraftMatcher::ifPossibleReduceByFamily(const CSimulatedAircraft &remoteAircraft,
                                                                  const QString &family, bool allowPseudoFamily,
                                                                  const CAircraftModelList &inList, const QString &hint,
                                                                  bool &reduced, CStatusMessageList *log)
    {
        ...
        if (family.isEmpty() && !allowPseudoFamily)
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, u"No family, skipping step (" % hint % u")",
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        if (inList.isEmpty())
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, u"No models for family match (" % hint % u")",
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        if (foundByFamily.isEmpty())
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    u"Not found by family '" % family % u"' (" % hint % ")");
            }
        ...
        else
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    u"Found by family '" % family % u"' (" % hint % u") size " %
                                                        QString::number(foundByFamily.sizeInt()),
                                                    getLogCategories());
            }
        }
        ...
        if (allowPseudoFamily)
        {
            ...
            const QString pseudo = remoteAircraft.getAircraftIcaoCode().getCombinedType() % "/" %
                                   remoteAircraft.getAircraftIcaoCode().getManufacturer();
            if (foundByCM.isEmpty())
            {
                if (log)
                {
                    CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                        u"Not found by pseudo family '" % pseudo % u"' (" % hint % ")");
                }
            }
            else
            {
                ...
                {
                    CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                        u"Found by pseudo family '" % pseudo % u"' (" % hint %
                                                            u") size " % QString::number(foundByCM.sizeInt()),
                                                        getLogCategories());
                }
            }
        }
        ...
        if (!foundByFamily.isEmpty())
        {
            foundByCM.removeModelsWithString(foundByFamily.getModelStringList(), Qt::CaseInsensitive);
        }
        ...
        {
            CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                u"Found by family (totally) '" % family % u"' (" % hint % u") size " %
                                                    QString::number(foundByFamily.sizeInt()),
                                                getLogCategories());
        }
    ...
    CAircraftModelList CAircraftMatcher::ifPossibleReduceByManufacturer(const CSimulatedAircraft &remoteAircraft,
                                                                        const CAircraftModelList &inList,
                                                                        const QString &info, bool &reduced,
                                                                        CStatusMessageList *log)
    {
        reduced = false;
        if (inList.isEmpty())
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, info % u" Empty input list, cannot reduce",
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        if (m.isEmpty())
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    info % u" No manufacturer, cannot reduce " %
                                                        QString::number(inList.size()) % u" entries",
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        if (outList.isEmpty())
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(
                    log, remoteAircraft, info % u" Not found '" % m % u"', cannot reduce", getLogCategories());
            }
            return inList;
        }
        ...
        {
            CMatchingUtils::addLogDetailsToList(
                log, remoteAircraft, info % u" Reduced by '" % m % u"' results: " % QString::number(outList.size()),
                getLogCategories());
        }
        reduced = true;
        return outList;
    ...
    CAircraftIcaoCodeList CAircraftMatcher::ifPossibleReduceAircraftIcaoByManufacturer(
        const CAircraftIcaoCode &icaoCode, const CAircraftIcaoCodeList &inList, const QString &info, bool &reduced,
        const CCallsign &logCallsign, CStatusMessageList *log)
    {
        reduced = false;
        if (inList.isEmpty())
        {
            if (log)
            {
                CCallsign::addLogDetailsToList(log, logCallsign, info % u" Empty input list, cannot reduce",
                                               getLogCategories());
            }
            return inList;
        }
        ...
        if (outList.isEmpty())
        {
            if (log)
            {
                CCallsign::addLogDetailsToList(log, logCallsign, info % " Not found " % m % ", cannot reduce",
                                               getLogCategories());
            }
            return inList;
        }
        ...
        {
            CCallsign::addLogDetailsToList(log, logCallsign,
                                           info % u" Reduced by " % m % u" results: " % QString::number(outList.size()),
                                           getLogCategories());
        }
        reduced = true;
        return outList;
    ...
    CAircraftModelList CAircraftMatcher::ifPossibleReduceByAircraft(const CSimulatedAircraft &remoteAircraft,
                                                                    const CAircraftModelList &inList,
                                                                    const QString &info, bool &reduced,
                                                                    CStatusMessageList *log)
    {
        ...
        if (inList.isEmpty())
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, info % " Empty input list, cannot reduce",
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        if (!remoteAircraft.hasAircraftDesignator())
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    info % " No aircraft designator, cannot reduce " %
                                                        QString::number(inList.size()) % " entries",
                                                    getLogCategories());
            }
            return inList;
        }

        const CAircraftModelList outList(
            inList.findByIcaoDesignators(remoteAircraft.getAircraftIcaoCode(), CAirlineIcaoCode::null()));
        if (outList.isEmpty())
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    info % u" Cannot reduce by '" %
                                                        ...
                                                        u"' results: " % QString::number(outList.size()),
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        if (log)
        {
            CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                info % u" Reduced by '" %
                                                    remoteAircraft.getAircraftIcaoCodeDesignator() % u"' to " %
                                                    QString::number(outList.size()),
                                                getLogCategories());
        }
        reduced = true;
        return outList;
    ...
    CAircraftModelList CAircraftMatcher::ifPossibleReduceByAircraftOrFamily(
        const CSimulatedAircraft &remoteAircraft, bool allowPseudoFamily, const CAircraftModelList &inList,
        const CAircraftMatcherSetup &setup, const QString &info, bool &reduced, CStatusMessageList *log)
    {
        reduced = false;
    ...
    CAircraftModelList CAircraftMatcher::ifPossibleReduceByAirline(const CSimulatedAircraft &remoteAircraft,
                                                                   const CAircraftModelList &inList,
                                                                   const CAircraftMatcherSetup &setup,
                                                                   const QString &info, bool &reduced,
                                                                   CStatusMessageList *log)
    {
        ...
        if (!remoteAircraft.hasAirlineDesignator())
        {
            ...
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    info % u" No airline designator, cannot reduce " %
                                                        QString::number(inList.size()) % u" entries",
                                                    getLogCategories());
            }
            return inList;
        ...
        CAircraftMatcherSetup::MatchingMode mode = setup.getMatchingMode();
        CAircraftModelList outList(
            inList.findByIcaoDesignators(CAircraftIcaoCode::null(), remoteAircraft.getAirlineIcaoCode()));
        if (mode.testFlag(CAircraftMatcherSetup::ByAirlineGroupSameAsAirline) ||
            ...
            if (remoteAircraft.getAirlineIcaoCode().hasGroupMembership())
            {
                ...
                if (log)
                {
                    CMatchingUtils::addLogDetailsToList(
                        log, remoteAircraft,
                        groupModels.isEmpty() ?
                            QStringLiteral("No group models found by using airline group '%1'")
                                .arg(remoteAircraft.getAirlineIcaoCode().getGroupDesignator()) :
                            QStringLiteral("Added %1 model(s) by using airline group '%2', all members: '%3'")
                                .arg(groupModels.sizeInt())
                                .arg(remoteAircraft.getAirlineIcaoCode().getGroupDesignator(),
                                     joinStringSet(groupModels.getAirlineVDesignators(), ", ")),
                        getLogCategories());
                }
            } // group membership
        }
        ...
        if (outList.isEmpty())
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                    info % u" Cannot reduce by '" %
                                                        remoteAircraft.getAirlineIcaoCodeDesignator() % u"' results: " %
                                                        QString::number(outList.size()),
                                                    getLogCategories());
            }
            return inList;
        ...
        if (log)
        {
            CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                info % u" Reduced by '" %
                                                    remoteAircraft.getAirlineIcaoCodeDesignator() % u"' to " %
                                                    QString::number(outList.size()),
                                                getLogCategories());
        }
        reduced = true;
    ...
    CAircraftModelList CAircraftMatcher::ifPossibleReduceModelsByAirlineNameTelephonyDesignator(
        const CCallsign &cs, const QString &airlineName, const QString &telephony, const CAircraftModelList &inList,
        const QString &info, bool &reduced, CStatusMessageList *log)
    {
        reduced = false;
        if (inList.isEmpty())
        {
            if (log)
            {
                CCallsign::addLogDetailsToList(log, cs, info % u" Empty input list, cannot reduce", getLogCategories());
            }
            return inList;
        }
        ...
        if (telephony.isEmpty() && airlineName.isEmpty())
        {
            ...
            {
                CCallsign::addLogDetailsToList(log, cs,
                                               info % u" No name/telephony, cannot reduce " %
                                                   QString::number(inList.size()) % u" entries",
                                               getLogCategories());
            }
            return inList;
        ...
        if (step1Data.isEmpty() || step1Data.size() == inList.size())
        {
            ...
            {
                CCallsign::addLogDetailsToList(
                    log, cs, info % QStringLiteral(" cannot reduce by '%1'").arg(airlineName), getLogCategories());
            }
            step1Data = inList;
        }
        else
        {
            ...
            if (log)
            {
                CCallsign::addLogDetailsToList(log, cs, info % QStringLiteral(" reduced by '%1'").arg(airlineName),
                                               getLogCategories());
            }
        }
        ...
        if (step2Data.isEmpty() || step2Data.size() == inList.size())
        {
            if (log)
            {
                CCallsign::addLogDetailsToList(log, cs, info % QStringLiteral(" cannot reduce by '%1'").arg(telephony),
                                               getLogCategories());
            }
            step2Data = step1Data;
        }
        else
        {
            ...
            if (log)
            {
                CCallsign::addLogDetailsToList(log, cs, info % QStringLiteral(" reduced by '%1'").arg(telephony),
                                               getLogCategories());
            }
        }
        return step2Data;
        ...
        {
            if (log) { CCallsign::addLogDetailsToList(log, cs, info % QStringLiteral(" cannot reduce by
        '%1'/'%2'").arg(airlineName, telephony), getLogCategories()); } return inList;
        }
    ...
    CAircraftModelList CAircraftMatcher::ifPossibleReduceByCombinedType(const CSimulatedAircraft &remoteAircraft,
                                                                        const CAircraftModelList &inList,
                                                                        const CAircraftMatcherSetup &setup,
                                                                        bool &reduced, CStatusMessageList *log)
    {
        ...
        if (!remoteAircraft.getAircraftIcaoCode().hasValidCombinedType())
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, QStringLiteral("No valid combined code"),
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        if (modelsByCombinedCode.isEmpty())
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, u"Not found by combined code " % cc,
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        {
            CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                u"Found by combined code " % cc % u", possible " %
                                                    QString::number(modelsByCombinedCode.size()),
                                                getLogCategories());
        }
        if (modelsByCombinedCode.size() > 1)
        {
            modelsByCombinedCode =
                ifPossibleReduceByAirline(remoteAircraft, modelsByCombinedCode, setup,
                                          QStringLiteral("Combined code airline reduction. "), reduced, log);
            modelsByCombinedCode =
                ifPossibleReduceByManufacturer(remoteAircraft, modelsByCombinedCode,
                                               QStringLiteral("Combined code manufacturer reduction. "), reduced, log);
            reduced = true;
        }
        return modelsByCombinedCode;
    }

    CAircraftModelList CAircraftMatcher::ifPossibleReduceByMilitaryFlag(const CSimulatedAircraft &remoteAircraft,
                                                                        const CAircraftModelList &inList, bool &reduced,
                                                                        CStatusMessageList *log)
    {
        reduced = false;
        ...
        if (byMilitaryFlag.isEmpty())
        {
            if (log)
            {
                CMatchingUtils::addLogDetailsToList(log, remoteAircraft, u"Models not found by " % mil,
                                                    getLogCategories());
            }
            return inList;
        }
        ...
        {
            CMatchingUtils::addLogDetailsToList(log, remoteAircraft,
                                                u"Models reduced to " % mil % u" aircraft, size " %
                                                    QString::number(byMilitaryFlag.size()),
                                                getLogCategories());
        }
        return byMilitaryFlag;
    }

    CAircraftModelList CAircraftMatcher::ifPossibleReduceByVTOLFlag(const CSimulatedAircraft &remoteAircraft,
                                                                    const CAircraftModelList &inList, bool &reduced,
                                                                    CStatusMessageList *log)
    {
        reduced = false;
        ...
        {
            CMatchingUtils::addLogDetailsToList(log, remoteAircraft, "Cannot reduce to VTOL aircraft",
                                                getLogCategories());
            return inList;
        }
        ...
        {
            CMatchingUtils::addLogDetailsToList(
                log, remoteAircraft, u"Models reduced to " % QString::number(vtolModels.size()) % u" VTOL aircraft",
                getLogCategories());
        }
        return vtolModels;
    }
    ...
    QString CAircraftMatcher::scoresToString(const ScoredModels &scores, int lastElements)
    {
        ...
        while (i.hasPrevious() && c++ < lastElements)
        {
            ...
            if (!str.isEmpty()) { str += '\n'; }
            str += QString::number(c) % u": score: " % QString::number(i.key()) % u" model: '" % m.getModelString() %
                   u"' aircraft: '" % m.getAircraftIcaoCodeDesignator() % u"' livery: '" %
                   m.getLivery().getCombinedCodePlusInfo() % u'\'';
        }
        return str;
    }

    CAirlineIcaoCode CAircraftMatcher::stringToAirlineIcaoObject(const CCallsign &cs, const QString &designator,
                                                                 const QString &airlineName,
                                                                 const QString &airlineTelephony, bool useSwiftDbData,
                                                                 CStatusMessageList *log)
    {
        if (!useSwiftDbData) { return { designator }; }
        if (!sApp || sApp->isShuttingDown() || !sApp->hasWebDataServices()) { return { designator }; }
        const CAirlineIcaoCodeList codes = sApp->getWebDataServices()->getAirlineIcaoCodesForDesignator(designator);
        if (codes.isEmpty()) { return { designator }; }
        if (codes.size() == 1) { return codes.front(); }

        // more than 1
        bool reduced = false;
        static const QString info("Try reducing airline '%1' by name/telephony '%2'/'%3'");
        const CAirlineIcaoCodeList reducedIcaos = codes.ifPossibleReduceNameTelephonyCountry(
            cs, airlineName, airlineTelephony, QString(), reduced, info, log);
